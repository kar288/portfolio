...
def getGrumbls(context, user, own = True):
	...
	context['grumblsJson'] = serializers.serialize('json', grumbls)

def grumbls_json(request):
	context = {}
	user = GrumblrUser.objects.filter(username = request.user)[0]
	
	getGrumbls(context, user)
	return render(request, 'grumbls_json.html', context, content_type = 'application/json')
...
def profile(request):
	...
	if request.method == 'POST':
		...
		if 'content' in request.POST:
			grumble = Grumble(content = postForm.cleaned_data['content'], user = user)
			grumble.save()
...
def other_user (request, name):
	...
	if not users or current_user in GrumblrUser.objects.filter(blockers=users[0]):
		raise Http404
...
def edit_information (request):
	...
	if request.POST:
		...
		try:
			user.save()
			context['success'] = 'Successfully changed'
...
def search(request):
	...
	context = {}
	getUserInfo(context, user, user)	
	#this is a hack: have to get full path with arguments too, how??
	context['url'] = request.META['PATH_INFO']

	if 'term' in request.GET and request.GET['term']:
		context['url'] += '?term=' + request.GET['term']
		grumbls = Grumble.objects.exclude(user__blockers__username = user.username).filter(Q(content__contains = request.GET['term']) | Q(user__username = request.GET['term'])).reverse()
